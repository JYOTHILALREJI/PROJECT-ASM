...
exports.buildNocFileName = buildNocFileName;
exports.monthKeyFromNocDate = monthKeyFromNocDate;
/**
...
const BLUE = (0, pdf_lib_1.rgb)(0, 0, 0.72);
...
function assetPath(name) {
    return path_1.default.join(process.cwd(), 'src', 'assets', 'noc', name);
}
...
function drawWrappedText(page, text, x, yTop, maxWidth, font, size, lineHeight, color = BLACK, align = 'left') {
...
function drawLetterhead(page, letterhead) {
...
async function generateNocPdf(data) {
    ...
    const font = await pdf.embedFont(pdf_lib_1.StandardFonts.TimesRoman);
    const fontBold = await pdf.embedFont(pdf_lib_1.StandardFonts.TimesRomanBold);
    const letterhead = await pdf.embedPng(fs_1.default.readFileSync(assetPath('letterhead.png')));
    const stampImg = data.stampType === 'signature'
        ? await pdf.embedPng(fs_1.default.readFileSync(assetPath('stamp-signature.png')))
        : data.stampType === 'none'
            ? null
            : await pdf.embedPng(fs_1.default.readFileSync(assetPath('stamp-procurement.png')));
    ...
    for (const part of bodyTexts) {
        for (const w of part.t.split(' ').filter(Boolean))
            tokens.push({ w, b: part.b });
    }
    let lineX = MARGIN;
...
function buildNocFileName(data) {
    const clean = (s) => s
        .replace(/[\\/:*?"<>|]+/g, ' ')
        .replace(/\s+/g, ' ')
        .trim();
...
function monthKeyFromNocDate(nocDate) {
